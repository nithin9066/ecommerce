<div class="bg-white">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:max-w-7xl lg:px-8">
      <h2 class="text-2xl font-extrabold tracking-tight text-gray-900">MOST POPULAR PRODUCTS</h2>
          ...
          <div class="mt-4 flex justify-between">
            <div>
              <h3 class="text-sm text-gray-700">
                <a href="/product/{{$pop->url}}">
                  <span aria-hidden="true" class="absolute inset-0"></span>
                  {{$pop->product_name}}
                </a>
              </h3>
              <p class="mt-1 text-sm text-gray-500">{{$pop->cat_name}}</p>
            </div>
            <p class="text-sm font-medium text-gray-900">₹{{$pop->our_price}}</p>
          </div>
        </div>
        @endforeach
